...
csv_logger = CSVLogger('resnet50_drivers.csv')
# color_type = 1 - gray
# color_type = 3 - RGB
...
def cache_data(data, path):
    if os.path.isdir(os.path.dirname(path)):
        #with h5py.File('path', 'w') as hf:
         #   hf.create_dataset("drivers", data=data)
        file = open(path, 'wb')
...
def create_submission(predictions, test_id, info):
    ...
    sub_file = os.path.join('../output/subm', 'submission_' + suffix + '.csv')
...
def create_model_v1(img_rows, img_cols, color_type=1):
    ...
    sgd = SGD(lr=0.001, decay=0, momentum=0, nesterov=False)
...
def resnet50_model(img_rows, img_cols, color_type=1):
    ...
    nb_classes = 10
    model = resnet.ResnetBuilder.build_resnet_50((color_type, img_rows, img_cols), nb_classes)
...
def keras_resnet50_model(img_rows, img_cols, color_type=1):

    nb_classes = 10

    model = ResNet50(include_top=True, input_shape =(color_type, img_rows, img_cols))
    model.layers.pop()
...
def run_single():
    ...
    model = keras_resnet50_model(img_rows, img_cols, color_type_global)
    #model = resnet50_model(img_rows, img_cols, color_type_global)
    ...
    predictions_valid = model.predict(X_valid, batch_size=4, verbose=1)
    ...
    test_prediction = model.predict(test_data, batch_size=4, verbose=1)
